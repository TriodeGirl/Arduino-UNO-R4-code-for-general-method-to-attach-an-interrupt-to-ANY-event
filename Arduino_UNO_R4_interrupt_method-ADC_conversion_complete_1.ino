 ...
 *
 *  Update 30 July - move pin-grab to D13, add clear pin settings (turn off pin IRQ function).
 *                 - Change analog input pin to A1
 *
...
//
// Example:   *MSTP_MSTPCRD &= (0xFFFFFFFF - (0x01 << MSTPD16));  // Enable ADC140 module
...
void setup()
  ...
  attachInterrupt(13, adcCompleteInterrupt, FALLING); // This IRQ will be asigned to Slot 05 IELSR05 as 0x001 PORT_IRQ0 - Table 13.4
  *ICU_IELSR05 = 0x029;                               // HiJack Slot 05 IELSR05 for ADC140_ADI
  *PFS_P111PFS = 0x00000000;                          // Clear D13 pin ISEL bit Interrupt Enable
...
void loop()
  ...
  *PFS_P106PFS_BY = 0x05;      // Pulse on D6 to trigger scope 
  *PFS_P106PFS_BY = 0x04;      //  
...
void setup_adc(void)  // Note Set to A1, A0 and A2 commented out
  {
